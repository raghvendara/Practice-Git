...
public class PersonManipulation {
	...
	public void deleteStudent()
	{
		...
		int rollNumber=read.nextInt();
		ArrayList<Student> list1= readWriteInfo.ReadFromFile();
		
		Iterator<Student> itr2 = list1.listIterator();
		int count=0,index=0;
		while (itr2.hasNext()) 
		{
			Student obj = (Student)itr2.next();
			int getRollNumber = obj.getRollNumber();
			if(getRollNumber==rollNumber)
			{
				index=list1.indexOf(obj);
				count++;
				itr2.remove();
...
//				list1.remove(obj);
		...
		{
			System.out.println("student got...at index :"+index);
			readWriteInfo.writeIntoFileInOverwriteMode((ArrayList<Student>) list1);
